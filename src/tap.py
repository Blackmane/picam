...
class Tap:
    ...
    '''
    def setSchermata(self, i): #?
        #self.idxS = i
        self.schermate.setView(i)
        print "set schermata"    # '''
    ...
    def timerStart(self, i):
        ...
        if self.tapDownObj is not None:
            if i == 0:    # (250, 2000) (Click Time, Press Time)
                if self.tapDownObj.DoubleClickFun is not None:
                    # Inutile attivare i timer se non c'è la funzione #?
                    self.doppioClickTimer = self.times[0]
            elif i == 1:
                if self.tapDownObj.onMouseMoveFun is not None:
                    # Inutile attivare i timer se non c'è la funzione #?
    ...
    def timerTest(self, currentTime):
        ...
        # Se nello stato 4    
            ...
            if self.doppioClickTimer <= 0:
                ...
                self.doppioClickTimer = -1
                if self.tapDownObj is not None: #?
                    self.tapDownObj.Click(self.tapPos)
                    #self.tapDownObj.esegui(f_click)
                self.stato = 0
    ...
    def onMouseDown(self, punto):
        ...
        # Da stato 4 a stato 0 -> DOPPIOCLICK"
            ...
            if self.tapDownObj is not None:
                ...
                #self.tapDownObj.esegui(f_dclick)
            self.stato = 0
        else:
            ...
            if dis > 5:
                ...
                if self.rightTap:
                    ...
                    if self.tapDownObj is not None:
                        ...
                        #self.tapDownObj.esegui(f_muovi)
                    self.stato = 5
                else:
                    # Da stato 1 a stato 2
                    # Da stato 2 a stato 2
                    # self.schermate.offset = diff
                    self.pressTimer = -1
